...
class Demo_Single_Matrix():
    ...
    def initialize_plot(self):
        ...
        t_dags = pn.pane.Markdown('''
                                ...
                                ''')
        t_interaction = pn.pane.Markdown('''
                                Select an intervention above to see the simulated data of the intervention.
        ...
        if self.status not in ["static"]:
            ...
            self.plot = pn.Column(pn.Column(*widget_boxes),pn.Column(pn.Column(t_graphs, t_interaction, widget.slider, grid_obs.get_grid()),pn.Column(t_dags, pn.Row(*cols))))
        else:
            widgets = []
            grid_obs = []
